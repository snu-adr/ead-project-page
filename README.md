...
### Node.js 설치 — Windows (WSL 사용)

Windows에서는 **WSL(Windows Subsystem for Linux)** 환경을 사용한다. 빌드 도구와의 호환성이 가장 좋고, 위의 Linux 가이드를 그대로 재사용할 수 있다.
...
설치 후 재부팅 → Ubuntu 실행 → 사용자 계정 생성 → 위의 **Linux/macOS 가이드를 그대로 따른다** (`nvm` 설치부터).
...
- **(Windows) `wsl --install` 실패**: Windows 10 2004 이상 / Windows 11 이어야 한다. PowerShell을 관리자 권한으로 열었는지 확인. 가상화 기능이 BIOS에서 꺼져 있다면 켠다.
- **(Windows) WSL 안에서 작업했는데 Windows 쪽 폴더가 느림**: 프로젝트는 반드시 WSL 홈 디렉토리(`~/`) 안에 두고 작업한다. `/mnt/c/...` 경로에서 작업하면 파일 I/O가 매우 느려진다.
